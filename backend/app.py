...
def upload_book():
    ...
    book_text, sentence_list, embedding_matrix = upload_and_process_book(file_name=file_name, book_in_text=book_text)
    return jsonify({'message': 'Book uploaded and processed', 'num_sentences': len(sentence_list), 'book_text': book_text}), 200
...
def similar_sentences():
    ...
    similar = find_similar_sentences_cosine(sentence, sentence_list, embedding_matrix)
